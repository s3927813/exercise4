...
/*Full Screen Button*/
/*I added a full screen button because that's the button I personally tend to 
reach out for the most in media players other than the play/pause button. At first I did not know how to create 
the button at all but I found the 'requestFullScreen command, I also searched up w3schools on how to properly integrate them
in the function and added the ms and webkit element so the full screen command can still work on different browsers.
I only edited the function part of the javascript at first and stupidly questioned why the button did not work. But then I figured out I have to edit the 
get element id command on top of this document and then added an addeventlistener command on click. 
I also made an svg of the escape full screen button whilst being stuck on why the button did not work but ended up not using it (its still on the icons file
tho) I just edited the values of the M L L thing on the full screen svg without fully understanding them because even after watching several tutorials, I still 
don't really understand how to properly work with  them (I kinda understand how it works bc I did manage to create the escape full screen svg, but like I definitely can't make one from scratch ;-;
I was about to add the icon on the code when I realised that once the media player go full screen, the buttons I have made disappeared so I just took it as an experiment on svg :> */
